...
このラボの作業をしている環境に、**mslearn-ai-vision** コード リポジトリを既にクローンしている場合は、Visual Studio Code で開きます。それ以外の場合は、次の手順に従って今すぐクローンしてください。
...
1. Visual Studio Code で、リポジトリを複製した **Labfiles/03-object-detection/training-images** フォルダ－にあるトレーニング画像を表示します。 このフォルダーには果物の画像が含まれています。
...
8. 最後の画像のタグ付けが完了したら、**[Image Detail]** エディターを閉じます。 **[Training Images]** ページの **[Tags]** で **[Tagged]** を選択して、タグ付けされたすべての画像を表示します。
...
4. Visual Studio Code の **Labfiles/03-object-detection** フォルダーの下で、言語の設定に応じて **C-Sharp** または **Python** フォルダーを展開します。
...
これでプロジェクト内の画像にタグを付けたので、モデルをトレーニングする準備ができました。 自分
...
1. Visual Studio Code で、**Labfiles/03-object-detection** フォルダーを参照し、使用する言語のフォルダー (**C-Sharp** または **Python**) で、**test-detector** フォルダーを展開します。
